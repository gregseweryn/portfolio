...
export const Header: React.FC<HeaderProps> = ({ currentPath, navigate }) => {
  ...
  const t = commonContent[language].header
  ...
  const renderLangSwitcher = (isCompact = false) => (
    <div
      role="group"
      aria-label="Language selector"
      className="inline-flex items-center border border-zinc-200/90 rounded-full p-0.5 bg-zinc-50 text-xs"
    >
      <button
        type="button"
        onClick={() => setLanguage('en')}
        className={`rounded-full transition-colors cursor-pointer font-medium ${
          isCompact ? 'px-2 py-0.5 text-[11px]' : 'px-2.5 py-0.5'
        } ${
          language === 'en'
      ...
      <header className="py-5 border-b border-gray-100 relative z-30">
        <div className="site-container flex items-center justify-between gap-4">
          <div className="flex flex-col">
            ...
            >
              <p className="text-md font-medium leading-none text-[#111111]">
                {t.brandTitle}
              </p>
              <p className="block text-sm text-zinc-600 mt-1">{t.brandSubtitle}</p>
            </button>
          </div>
          ...
          <div className="sm:hidden flex items-center gap-2">
            ...
            <button
              ...
              className="px-3 py-1.5 text-sm font-medium border border-zinc-200 rounded-lg text-[#111111] hover:bg-zinc-50 active:bg-zinc-100 transition-colors cursor-pointer flex items-center gap-2 focus:outline-none focus-visible:ring-2 focus-visible:ring-[#111111]"
            >
              <span className="w-2 h-2 rounded-full bg-emerald-600"></span>
              <span>{mobileMenuOpen ? t.menuClose : t.menuOpen}</span>
            </button>
          </div>
        </div>
      </header>
      ...
      >
        <div className="site-container flex flex-row items-center justify-between gap-2">
          ...
          <div className="hidden sm:flex items-center gap-6 shrink-0">
            <nav aria-label="Sticky navigation" className="flex items-center gap-6 shrink-0">
              {navLinks.map((link) => {
                const isActive = currentPath === link.path
                return (
                  <a
      ...
      >
        <div className="flex items-center justify-between pb-4 border-b border-zinc-100">
          <div>
            ...
            <p className="text-xs text-zinc-500">{t.mobileSubtitle}</p>
          </div>
          <button
            onClick={() => setMobileMenuOpen(false)}
          ...
          {navLinks.map((link) => {
            ...
            return (
              ...
              >
                ...
                {isActive && <span className="text-xs font-semibold text-emerald-700 bg-emerald-50 px-2 py-0.5 rounded">{t.active}</span>}
              </a>
            )
          })}
        </nav>

        <div className="py-3 border-t border-zinc-100 flex items-center justify-between">
          <span className="text-xs font-medium text-zinc-500">
            {language === 'pl' ? 'Wersja językowa:' : 'Language:'}
          </span>
          {renderLangSwitcher()}
        </div>

        <div className="pt-3 border-t border-zinc-100 flex flex-col gap-2 text-xs text-zinc-500">
